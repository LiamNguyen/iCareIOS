...
class BookingManagerViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func bindAppointmentDataSource() {
        if let appointments = Functionality.pulledStaticArrayFromUserDefaults(forKey: (DTOCustomerInformation.sharedInstance.customerInformationDictionary["userId"] as! NSNumber).stringValue) as? DTOCustomerInformation {
