...
class BookingManagerViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func bindAppointmentDataSource() {
        if let appointments = Functionality.pulledStaticArrayFromUserDefaults(forKey: DTOCustomerInformation.sharedInstance.customerInformationDictionary[Constants.JsonPropertyName.userId] as! String) as? DTOCustomerInformation {
